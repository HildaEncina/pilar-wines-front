...
import {
  ...
  editarCarritoPorID,
  obtenerCarritoPorID,
} from "../Carrito/carritoSlice";
import axios from "axios";

const ProductoDetalle = () => {
  /** Mi producto */
  const { productoSeleccionado, loading, error } = useSelector(
    (state) => state.producto
  );

  /** ID Cliente */
  const { userId, rol } = useSelector((state) => state.login);
  ...
  const agregarAlCarrito = async () => {
    ...
    if (!carrito) {
      ...
      try {
        console.log(
          "Estoy dentro del try y soy un producto",
  ...
  const handleEditar = () => {
    console.log("Editar producto:", productoSeleccionado._id);
    navigate(`/editar-producto/${productoSeleccionado._id}`); 
  };
  ...
  const handleEliminar = async () => {
    ...
    try {
      ...
      const response = await axios.delete(
        `http://localhost:8082/api/producto/eliminar/${productoSeleccionado._id}`
      );
        ...
        <Col md={6} className="product-info">
          ...
          {rol === "administrador" && (
            <div className="admin-actions">
              ...
              <Button onClick={handleEliminar} className="delete-button">
                Eliminar
              </Button>
            </div>
          )}
